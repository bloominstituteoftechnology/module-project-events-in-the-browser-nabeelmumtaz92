...
function moduleProject2() {
  ...
  document.addEventListener('keydown', evt => {
    ...
    let isUp = evt.key === key.up
    let isDown = evt.key === key.down
    let isLeft = evt.key === key.left
    let isRight = evt.key === key.right
    ...
    // 👉 TASK 5 - End the game 👈
        let liveMosquitoes = document.querySelectorAll('[data-status=alive]') {
          if (!liveMosquitoes.length) {
            ...
            document.querySelector('p.info').textContent= `Extermination compelted in ${elapsed/1000} seconds!`
